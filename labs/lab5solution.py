...
def get_data(paths):
    ...
    for path in paths:
        for file in os.listdir(path):
            fname = path + '/' + file
            infile = open(fname, errors='ignore')
            X.append(infile.read())
            if 'nonfiction' in path:
                y.append(0)
            else:
                y.append(1)
            infile.close()
...
class NaiveBayesClassifier:
    ...
    def count_features(self):
        ...
        for i in range(len(self.X)):
            for feature in self.tokenize(self.X[i]):
    ...
    def get_tokens(self, text):
        ...
        return random.sample(sorted(text), k) # sorted so we are certain 
        # that we can set a seed and get reproducible results for testing.
    ...
    def fiction_prob(self, text):
        ...
        '''
        fv = self.tokenize(text)
 
        fv = self.get_tokens(fv)
        log_p_if_f = 0.0
        log_p_if_n = 0.0
        for feature in fv:
...
def main():
    ...
    '''
    paths = ['books/fiction','books/nonfiction']
    data = get_data(paths)
    
    #shuffle the data and split into train and test
    data = shuffle_data(data[0],data[1])
